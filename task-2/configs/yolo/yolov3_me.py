...
train_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='annotations/train2007.json',
        data_prefix=dict(img='train2024/'),
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='annotations/val2007.json',
...
test_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='annotations/test2007.json',
        data_prefix=dict(img='test2024/'),
        test_mode=True,
        pipeline=test_pipeline,
        backend_args=backend_args))
...
test_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'annotations/test2007.json',
    metric='bbox',
    format_only=True,  # 只将模型输出转换为 coco 的 JSON 格式并保存
    backend_args=backend_args)

train_cfg = dict(max_epochs=4, val_interval=1)
